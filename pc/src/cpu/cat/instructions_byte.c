...
{
    CPU_ASSERT(OPERAND_IS_WRITABLE(0) && OPERAND_IS_READABLE(1),
...
{
    CPU_ASSERT(OPERAND_IS_WRITABLE(0) && OPERAND_IS_ADDRESSABLE(1),
               CPU_ERROR_ILLEGAL_OPERAND);
...
{
    CPU_ASSERT(OPERAND_IS_WRITABLE(0) && OPERAND_IS_WRITABLE(1),
               CPU_ERROR_ILLEGAL_OPERAND);
...
void cpu_cat_8_jump_if_greater(struct cpu_execution_state *state)
{
    ...
    if (!state->cpu->state.regs.flags.zero &&
        (state->cpu->state.regs.flags.sign ==
         state->cpu->state.regs.flags.overflow))
        state->cpu->state.regs.pc = operand_read(state, 0);
}
