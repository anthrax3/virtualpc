...
i_loadimm8(struct cpu_s *in)
{
    ...
    uint8_t imm;
    reg = memory_read(in->mem, ++in->state.regs.pc);
    imm = memory_read(in->mem, ++in->state.regs.pc);
...
i_add(struct cpu_s *in)
{
    ...
    reg1 = memory_read(in->mem, ++in->state.regs.pc);
    reg2 = memory_read(in->mem, ++in->state.regs.pc);
